...
export function tripleNumbers(numbers: number[]): number[] {
    let tripled: number[] = numbers.map((nums: number): number => nums * 3);
    return tripled;
}
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    let newmessages: string[] = messages.map((message: string) =>
        message.at(-1) === "!" ? message.toUpperCase() : message,
    );
    let removeQuestion: string[] = newmessages.filter(
        (message: string) => message.at(-1) !== "?",
    );
...
export function countShortWords(words: string[]): number {
    let shortWords: string[] = words.filter((word: string) => word.length < 4);
